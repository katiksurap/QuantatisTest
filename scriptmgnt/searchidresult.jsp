        ...
        <% // if ((request.getParameter("sdate") != null) && (request.getParameter("edate") != null)) {
             ...
             String stdate = request.getParameter("sdate");
                ...
                if (insrt_art_in_session == 1) {
                    art = pgn.showdetailsscriptidwise(search,edate,status,user_type);
...
filesize=fs.getsize(pgb.getScriptUpload_path());
   ...
   <td> <a href="download.jsp?fname=<%=pgb.getScriptUpload_fname()%>&userid=<%=pgb.getUser_id()%>" style="color: #A42300">
                     <%=pgb.getScriptUpload_fname()%><br><span style="color: red">File Size Is <%=filesize%> </span><br></a></td>
            ...
            System.out.println("createdby---"+pgb.getUsername());
                        ...
                        if (art.size() > 0) {
                            ...
                            out.println(paging.FooterHtml(PageNo, art.size(), MNRPP, "searchidresult.jsp",stdate,edate));
                        }
                    }
                //}
            %>
      </table><br> <table align="center"><td><a  href="<%=response.encodeURL("ScriptStatus.jsp")%>"><input type="button" value="Go Back" class="btn" ></a></td></table>
